...
List<Map<String, dynamic>> processAll=[];
  ...
  List<String> getProcessus(List<Map<String, dynamic>> liste) {
    ...
    for (var element in liste) {
      if(tr.abrLange.toLowerCase() =='en'){
         ...
         if (element.containsKey("nom_processus") && element["nom_processus"] != null) {
    ...
    Future<List<ContributeurModel>> translateEnProcessContributeurs(List<ContributeurModel>contributeurs) async {
  ...
  void refreshData() async {
    ...
    final definiivereponse = tr.abrLange.toLowerCase() =='en' ? await translateEnProcessContributeurs(response) : response;
...
class ContributeurDataGridSource extends DataGridSource {
  ...
  Widget getAcces(AccesPilotageModel acces) {
    if (acces.estBloque == true) {
      return  Text(tr.typeacccesList("bloque"),
...
class _AjoutContributeurState extends State<AjoutContributeur> {
  ...
  static List<String> listAcces = [tr.typeacccesList('spectator'),tr.typeacccesList('editor'),tr.typeacccesList('validator'),tr.typeacccesList('admin')];
                          ...
                          : () async {
                              if (_formKey.currentState!.validate() &&
                                ...
                                final userAcces = {
                                  ...
                                  "est_admin": false,
                                  "processus": await OperationList().getProcessFr(selectedProcess) ,
                                };
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    ...
                                    content: Text(tr.accountHasCreated(emailEditingController.text),
                                      style: const TextStyle(fontSize: 16),
                                    ),
                                  ),
                                );
